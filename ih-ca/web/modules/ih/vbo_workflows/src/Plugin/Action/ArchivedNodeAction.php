 ...
 *
 * @Action(
 *   id = "vbo_workflows_archive_node_action",
 *   label = @Translation("Archive node (moderation_state)"),
 *   type = "node",
 *   confirm = TRUE
 * )
 */
class ArchivedNodeAction extends ViewsBulkOperationsActionBase {
  ...
  public function execute(ContentEntityInterface $entity = NULL) {
    ...
    if (!empty($state)) {
      if ($state == 'published') {
        ...
        // Set the moderation state as archived.
        ...
        $this->messenger()->addWarning(t('%title  - cannot change state directly from %state to archived', [
          '%title' => $entity->getTitle(),
          '%state' => $state,
        ]));
      }
